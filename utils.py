...
def get_module_names_shapes(model, return_modules=False, filter_type=True):
    module_names = []
    module_shapes = []
    modules = []
    for name, module in model.named_modules():
        ...
        if len(module.weight.shape) == 1: continue
        if ("attn" in name or "attention" in name) and len(module.weight.shape) == 2:
...
def net_esd_estimator_full(
    ...
    # print("=================================")
    # iterate through layers
    module_names, module_shapes, modules = get_module_names_shapes(net, return_modules=True, filter_type=False)
    # for name, m in net.named_modules():
    for name, m in zip(module_names, modules):
        if name in results["longname"]: continue
        # if isinstance(m, nn.Conv2d) or isinstance(m, nn.Conv1d) or isinstance(m, nn.Linear):
        ...
        eigs_all.append(nz_eigs.cpu().numpy())
        results['D'].append(final_D)
        results['M'].append(m.weight.shape[0])
        results['N'].append(m.weight.shape[1])
        results['alpha'].append(final_alpha)
        ...
        if model_name and save_dir:
            ...
            save_path = os.path.join(save_dir, "_%s.csv"%(model_name.replace('/', '--')))
            layer_stats.to_csv(save_path)
            np.save(os.path.join(save_dir, "%s.npy"%(model_name.replace('/', '--'))), eigs_all)
...
def create_input_prompt(args, fewshot_path=None, prompting=False)->str:
    x, z, y = [], [], []
    if prompting:
        assert fewshot_path is not None
